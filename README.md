...
**처음 시작하시나요?** 아래 가이드를 순서대로 읽어보세요!

### 🎯 이해하기 (코드 없이 텍스트만)
1. 📋 **[API 기능 설명서](./API_기능_설명서.md)** - 각 API가 어떤 데이터를 제공하고 무엇을 할 수 있는지 한눈에 보기

### 💻 사용하기 (개발자용 코드 포함)
2. ⚡ **[빠른 시작 가이드](./quick_start_guide.md)** - 5분 안에 첫 API 호출하기
3. 📖 **[API 사용 예제](./example_usage.md)** - 주요 API 6가지 실전 코드
4. 📊 **[데이터 사용 범위 가이드](./data_usage_guide.md)** - 어떤 데이터를 어디까지 사용할 수 있나요?
5. 📚 **[카테고리별 상세 가이드](./api_detailed_guide.md)** - 모든 API의 파라미터와 응답 형식
