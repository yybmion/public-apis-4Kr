...
└─────────────────────────────────────────────────────────┘
        ...
        ↓                 ↓                 ↓             ↓
┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│ KIS API      │  │ Yahoo        │  │ DART API     │  │ FRED API     │
│ (한국 주식)   │  │ Finance      │  │ (재무제표)    │  │ (미국 경제)   │
│              │  │ (미국 지수)   │  │              │  │ 80만+ 지표   │
└──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘
        │                 │                 │             │
        ├─────────────────┼─────────────────┼─────────────┤
        ↓                 ↓                 ↓             ↓
┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│ ECOS API     │  │ Fear & Greed │  │ WallStreet   │  │ StockTwits   │
│ (한국 경제)   │  │ Index        │  │ Bets API     │  │ API          │
│ 10만+ 지표   │  │ (시장 심리)   │  │ (소셜 트렌드) │  │ (투자 감성)   │
└──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘
                          │
        ┌─────────────────┼─────────────────┐
        ↓                 ↓                 ↓
┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│ Multi-LLM    │  │ Supabase     │  │ Redis        │
│ Analysis     │  │ (PostgreSQL) │  │ (캐시)        │
│ (4개 Agent)  │  │ 20개 테이블   │  │              │
└──────────────┘  └──────────────┘  └──────────────┘
```
...
### Phase 1: MVP + 경제 데이터 통합 (Week 1-4) ✅

- [x] **데이터 수집 인프라**
  - 한국투자증권 KIS API 연동
  - Yahoo Finance 미국 지수 연동
  - DART 재무제표 API 연동
  - **FRED API 연동 (80만+ 미국 경제 지표)** ✨
  - **ECOS API 연동 (10만+ 한국 경제 지표)** ✨
  - **Fear & Greed Index 수집 (시장 심리)** ✨
  - PostgreSQL 데이터베이스 구축 (20개 테이블)

- [x] **웹 인터페이스**
  - FastAPI RESTful API 서버 (33+ 엔드포인트)
  - Streamlit 대시보드
  - Docker 컨테이너화
  - 실시간 시장 현황 표시
  - **경제 스냅샷 대시보드** ✨
  - **Yield Curve 분석 및 경기 침체 신호** ✨
...
| 미국 경제 지표 | FRED | 미 연방준비은행 | ⭐⭐⭐⭐⭐ |
| 한국 경제 지표 | ECOS | 한국은행 | ⭐⭐⭐⭐⭐ |
| 시장 심리 | Fear & Greed Index | CNN | ⭐⭐⭐⭐ |
| 뉴스 | 빅카인즈 | 한국언론진흥재단 | ⭐⭐⭐⭐ |
...
│   │   ├── fred_collector.py    # FRED API (80만+ 미국 경제 지표) ✨
│   │   ├── ecos_collector.py    # ECOS API (10만+ 한국 경제 지표) ✨
│   │   ├── fear_greed_collector.py  # Fear & Greed Index (시장 심리) ✨
